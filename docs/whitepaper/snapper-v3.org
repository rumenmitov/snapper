...
| BYTE RANGE | PURPOSE                  |
|------------+--------------------------|
| 0 to 31    | SHA-256 of encoded data. |
| 32 to end  | Encoded data.            |

Note that while the SHA-256 of the archive file can detect a corrupted archive file, Snapper 3.0 does not have any way to fix the file. It is recommended to make regular backups of the latest version of the archive file. It is important that said backups are done while Snapper is inactive (i.e. not in the middle of an operation) and the file-system is consistent with the archive file (=/<snapper-root>/WAL= is not present).

Replacing a corrupted archive file with an outdated backup is not supported and is considered undefined behavior.

Layout of encoded data:

| BYTE RANGE                                           | PURPOSE             |
|------------------------------------------------------+---------------------|
| 32 to (size_{payloadmap} - 1)                           | Encoded payloadmap. |
...
*** Redundancy-Ratio
The redundancy-ratio is the relationship between the number of distinct payloads, \langle =snapid=, =chunkid= \rangle, that require a file from a redundant set and the size of that redundant set. A low redundancy-ratio is an indicator that more redundant copies should be created (see [[#configuration][Configuration]]).
...
Note that due to how redundant copies are named, the implementation does not allow for =Snapper::Config::redundancy= to exceed 1.0.
...
6. For each payload \langle =snapid=, =chunkid= \rangle:
   ...
   4. Choose a snapshot file from each of those sub-directories. Check that the snapshot file matches the hash. Compare the snapshot file with the contents of the chunk. Note, that if the snapshot file does not correspond to the hash, it should be fixed by replacing it with one of its redundant copies. If no such copy exists, the redundant set should be removed.
...
2. For each requested \langle =snapid=, =chunkid= \rangle in =Snapper::archive::payloadmap=, use the resulting =rfid= to:
   ...
   2. Select any snapshot file from the redundant file sub-directory, where the snapshot file matches the hash. If the snapshot file does not match the hash, it is "corrupted" and should be replaced by another redundant copy. If no other redundant copy exists, an irrecoverable corruption error should be returned.
...
2. For each =rfid= entry in =snapid='s entry in =Snapper::archive::snapmap=:
   ...
   2. Remove all =Snapper::archive::payloadmap= entries that contain =snapid=.
...
6. For each =rfid= in =Snapper::archive::filemap=:
   ...
   3. If =rc= is greater than 0:
      ...
      2. If there are no file paths: signal a data loss error, remove the sub-directory, remove the =rfid= entry from =Snapper::archive::filemap=, and continue to next =rfid= (/step 6/).
...
- Let n be the maximum count of redundant sets under any hash sub-directory.
- Let \omega be the maximum count of redundant copies in any redundant set.
...
Keep in mind that the complexity of \[ O(s \cdot c) \] assumes that each snapshot always creates a new snapshot file for each chunk. With redundant files in practice this should reduce the amortized complexity based on how often the contents of the chunk change. Hashing and file-system lookups are also assumed to be constant time (see [[#expected-workloads][Expected Workloads]]).
...
#+ATTR_LATEX: :font \footnotesize
  ...
  | =Snapper::capture()= | O(s \cdot c + n \cdot \omega)[fn:5] | O(s \cdot c + M)     | On every payload saved.          |
  | =Snapper::commit()=  | O(s \cdot c)               | O(s \cdot c)         | Once per new snapshot.           |
  |--------------------+------------------------+------------------+----------------------------------|
  | =Snapper::load()=    | O(s \cdot c)               | O(s \cdot c)         | Once when restoring a snapshot.  |
  | =Snapper::restore()= | O(\omega)                   | O(s \cdot c + M)     | On every payload restored.       |
...
There are three operations that could result in a change in the redundancy-ratio of a given payload: capturing, purging, and healing a snapshot. The purge and heal operations include a check to ensure that the redundancy-ratio of a payload will not drop below =Snapper::Config::redundancy= before removing a snapshot file from the redundant file set. Hence, the property holds trivially.
...
Therefore, the total memory cost, Cost_{mem}, will be: $p \cdot 32 + |R| \cdot 24$ bytes. Each time a new redundant set needs to be created for a payload, the cost will be 56 bytes. Conversely, if a payload is part of an already existing redundant set, the cost will be 32 bytes.

\[ Cost_{mem} (p, |R|) = 56 \cdot |R| + 32 \cdot (p - |R|) \]
...
:PROPERTIES:
:CUSTOM_ID: expected-workloads
:END:
Keeping in mind the previously-outlined limitations, Snapper 3.0 uses 64-bit hashes, with 2 bytes (i.e. hexadecimal characters) per directory level. This results in a maximum count of directory entries of 256, and 8 levels of sub-directories.

With a hash space of 64-bits, a hash collision rate of 0.03% can be achieved with a number of redundant sets, |R|, of 10^{8}. In the worst case when all payloads are unique:

\[ Cost_{mem} (p = 10^{8}, |R| = 10^{8}) = 56 \cdot 10^{8} bytes = 5.6 GB \].
...
The relationship for a given p (in this case p = 10^{8}) and number of redundant sets (relative to the stated p value), can be visualized as follows:
